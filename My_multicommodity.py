...
# Max_lenght_path_constraints
Max_path_lenght_allowed=0
for h in commodities:
   prob += pulp.lpSum(flow[h, o, d] for o, d, data in graph.edges(data=True)) <= Max_path_lenght_allowed, "Lenght_path_between_(%s,%s)" % (str(commodities[h]["from"]),str(commodities[h]["to"]))
